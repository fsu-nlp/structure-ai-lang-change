# this code calculates the percent change per token and significance, removing tokens of not relevant POS
...
def count_words(text):
    ...
    for line in text:
        ...
        words = line.split()
        for word in words:
            if any(keyword in word for keyword in ["_PUNCT", "_SYM", "_NUM", "_X", "_PROPN"]) or not word.split("_")[0].isalnum():
                continue
            if word in freq_dct:
                freq_dct[word] += 1
            else:
...
text2020 = open( 'pubmed_sample_pos.txt', 'r', encoding='utf-8') # input_human_abstracts_sample_2020.txt
...
text2024 = open('ai_abstracts_pos.txt', 'r', encoding='utf-8') # output_ai_abstracts-from-summary_gpt-3.5-turbo-instruct.txt
...
output_file = open('ai_abstracts_brute_force.txt', 'w', encoding='utf-8') # brute_force_divergence_human_vs_ai-summary-based-abstracts
output_file.write("word\tchange (%)\topm BASE\topm INST\tsignificant\n")
